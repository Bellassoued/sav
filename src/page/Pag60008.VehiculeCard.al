page 60008 "Vehicule Card"
{
    // version CT16V002

    CaptionML = ENU='Vehicule Card',
                FRA='Fiche Véhicule';
    DelayedInsert = false;
    PageType = Card;
    ...
    SourceTableView = SORTING(No.)
                      ORDER(Ascending)
                      WHERE(Item type=CONST(Vehicule),
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Model en cours";"Model en cours")
                {
                    Caption = 'Modèle Actif/Non Actif';
                }
                field("Default Interrior";"Default Interrior")
                {
                }
                ...
                field("Inventory avant douane";"Inventory avant douane")
                {
                }
                field("Inventory après douane";"Inventory après douane")
                {
                }
            }
            group(Invoicing)
            {
                ...
                field("Age Réservation VN (jours)";"Age Réservation VN (jours)")
                {
                }
                field(Commission;Commission)
                {
                    Editable = GBoolCommissionEditable;
                }
            }
            group(Challenge)
            {
                field("Challenge No.";"Challenge No.")
                {
                }
            }
            group(Replenishment)
    ...
    actions
    {
        area(navigation)
        {
            group("Master Data")
            {
                ...
                Image = "<DataEntry>";
                action("Nomenclature Service Rapide")
                {
                    Image = AssemblyBOM;
                    Promoted = true;
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                    RunObject = Page 60061;
                    RunPageLink = Vehicule No.=FIELD(No.);
                }
                ...
                action("Temps standard services")
                {
                    Image = Timesheet;
                    Promoted = true;
                    PromotedCategory = Process;
                    ...
                    RunPageLink = Vehicule Model=FIELD(No.);
                }
                ...
                action("Page Fiche Technique")
                {
                    CaptionML = ENU='Fiche Technique',
                                FRA='Fiche Technique';
                    Image = Card;
                    Promoted = true;
                    PromotedIsBig = true;
    ...
    begin
        ...
        //SM 2017
        Type := Type::Inventory;
        "Item type":="Item type"::Vehicule;
        "Inventory Posting Group" := 'VEHICULES';
        //SM 2017
    end;
    ...
    begin
        ...
        GBoolCommissionEditable:=UserSetup."Autoriser mise a jour prix";
    end;
    ...
    var
        ...
        GBoolCommissionEditable : Boolean;
