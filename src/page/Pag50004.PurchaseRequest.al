page 50004 "Purchase Request"
{
    ...
    layout
    {
        area(content)
        {
            group("Général")
            {
                ...
                field("En Attente d'approbation D.G";"En Attente d'approbation D.G")
                {
                    Editable = false;
                    Style = Attention;
                    StyleExpr = TRUE;
                ...
                field("Refusé par la Dir.Genenal";"Refusé par la Dir.Genenal")
                {
                }
                field("Ré-Ouvrir par Dir.Genenal";"Ré-Ouvrir par Dir.Genenal")
                {
                }
                field("Motif Refus Dir. Général.";"Motif Refus Dir. Général.")
                {
                }
                field("Nature de l'achat";"Nature de l'achat")
                {
                    Visible = NotDG;
                }
                ...
                field("Mode de règlement";"Mode de règlement")
                {
                    Visible = NotDG;
                }
    ...
    actions
    {
        area(processing)
        {
            ...
            group(Fonctions)
            {
                ...
                action(Lancer)
                {
                    ...
                    var
                        ...
                        PurchaseRequestLine1 : Record "50008";
                        PurchaseRequestLine2 : Record "50008";
                    begin
                        ...
                        /*
                            CLEAR(PurchaseRequestLine1);
                            PurchaseRequestLine1.SETFILTER(PurchaseRequestLine1."Request No.",'%1',"No.");
                            PurchaseRequestLine1.FINDFIRST;
                            REPEAT
                                  CLEAR(PurchaseRequestLine2);
                                  PurchaseRequestLine2.SETFILTER("Request Date",'%1..%2',DMY2DATE(1,1,DATE2DMY("Request Date",3)),DMY2DATE(31,12,DATE2DMY("Request Date",3)));
                                  PurchaseRequestLine2.SETFILTER("Vendor No.",'%1',PurchaseRequestLine1."Vendor No.");
                                  PurchaseRequestLine2.SETFILTER("Item No.",'%1',PurchaseRequestLine1."Item No.");
                                  PurchaseRequestLine2.SETFILTER(Montant,'%1',PurchaseRequestLine1.Montant);
                                  IF PurchaseRequestLine2.FINDFIRST THEN
                                   "Dépassement Budget"  := 'Demande No.: ' + PurchaseRequestLine1."Request No." +  ' existe avec le même Fournisseur ' +
                                                                                 PurchaseRequestLine2."Vendor No." + ' , même montant ' +
                                                                                  FORMAT(PurchaseRequestLine2.Montant) + '  même année ' + FORMAT(DATE2DMY(PurchaseRequestLine2."Request Date",3));
                        
                                   PurchaseRequestLine2."Dépassement budget" := FALSE;
                            UNTIL PurchaseRequestLine1.NEXT = 0;
                        ...
                        MESSAGE('  InclureBud  %1',InclureBud);
                        //contrôle budget
                           ...
                           BEGIN
                             ...
                             GLBudgetEntry.FINDFIRST;
                              ...
                              encours := GLAccount1."D.A M.T Consommé";// + GLAccount1."Mnt CMD-ACH facturé/DA" +GLAccount1."Mnt CMD-ACH non réceptionné/DA"+GLAccount1."Mnt CMD-ACH réceptionné/DA";
                               ...
                               IF USERID = 'BSB\ADMINISTRATEUR' THEN
                                   MESSAGE('  encours  %1   MontantBudgetLigne  %2',encours,MontantBudgetLigne);
                             ...
                             //    MESSAGE('DA %1   Fact %2 Cmd %3 réception  %4  %5',GLAccount1."D.A M.T Consommé",GLAccount1."Mnt CMD-ACH facturé/DA",GLAccount1."Mnt CMD-ACH non réceptionné/DA",
                               ...
                               IF (encours + PurchaseRequestLine.Montant) > MontantBudgetLigne  THEN
                                  BEGIN
                                      MESSAGE(CstGText015);
                                  ...
                                  IF USERID = 'BSB\ADMINISTRATEUR' THEN
                                   MESSAGE('  "Dépassement Budget"  %1',"Dépassement Budget");
                                  END;
                ...
                action("Créer Commande")
                {
                    ...
                    begin
                        ...
                        IF "En Attente d'approbation D.G" THEN
                          ERROR(CstGText050);
                ...
                action("Approbation D.Financier")
                {
                    ...
                    begin
                        ...
                        */
                        ...
                        IF CONFIRM('ëtes vous sûre de vouloir envoyer cette demande pour approbation D.G',TRUE,FALSE) THEN
                        BEGIN
            ...
            group("Approbation Direction Générale")
            {
                action("Envoyer Pour Approbation D.G")
                {
                    Image = SendTo;
                ...
                action("Annuler Envoi Demande App D.G")
                {
                    Image = Cancel;
            ...
            group(Imprimer)
            {
                ...
                action("Importer lignes autre demande")
                {
                    ...
                    var
                        ...
                        LRecPurchaseRequestLine2 : Record "50008";
                        NextLineNo : Integer;
                    begin
                        ...
                        IF LRecPurchaseRequestHeader.FINDSET THEN BEGIN
                          ...
                          IF LPagePurchaseRequestList.RUNMODAL=ACTION::LookupOK THEN BEGIN
                            ...
                            LRecPurchaseRequestLine.SETRANGE("Request No.","No.");
                            IF LRecPurchaseRequestLine.FINDLAST THEN
                              NextLineNo := LRecPurchaseRequestLine."Line No." + 1000
                            ELSE
                              NextLineNo := 1000;
                            //END_KK
                            LRecPurchaseRequestLine.RESET;
                            ...
                            IF LRecPurchaseRequestLine.FINDSET THEN BEGIN
                              REPEAT
                                ...
                                //KK_19112019
                                LRecPurchaseRequestLine_insert."Line No." := NextLineNo;
                                NextLineNo += 1000;
                                //END
                            ...
                            //KK_19112019
                            ELSE
                              MESSAGE('Aucune ligne non approuvée trouvée dans la demande %1',LRecPurchaseRequestHeader."No.");
                            //END
    ...
    var
        ...
        NotDG : Boolean;
        CstGText018 : TextConst ENU='The request must be Checked by finance',FRA='Une approbation de la direction générale est manquante pour cette demande';
        CstGText050 : TextConst ENU='The request must be Checked by finance',FRA='Cette demande est réfusée par la direction générale';
